...
Standalone variant of ``continual_opm.py`` that uses the in-tree
...
package. Runs are written under ``runs/continual_opm_standalone*/`` to
avoid colliding with the original pipeline.

Usage:
    python -m pipeline.continual_opm_standalone opm [--ordering_name ord0] [--model_size 150M] [--dry_run]
    python -m pipeline.continual_opm_standalone status [--condition all]
    python -m pipeline.continual_opm_standalone retry --condition opm --ordering_name ord0 --stage 2 --substage old_probe
"""
...
from pipeline.olmix_fit import (
    fit_and_propose,
    write_csvs,
)
...
class ExplicitOldProbeExperiment:
    ...
    def __init__(
        ...
        suffix = _run_suffix()
        self.run_dir = RUNS_ROOT / f"continual_opm_standalone{suffix}" / ordering_name / model_size
...
def status(condition="all"):
    ...
    for cond in conditions:
        root = RUNS_ROOT / f"continual_{cond}_standalone"
        if not root.exists():
...
def retry(condition, ordering_name, stage, substage=None, model_size="150M"):
    root = RUNS_ROOT / f"continual_{condition}_standalone" / ordering_name / model_size
